...
export function VerticalImageStack() {
  ...
  const getCardStyle = (index: number) => {
    ...
    if (diff === 0) {
      ...
      return { y: isMobile ? -100 : -160, scale: 0.82, opacity: 0.6, zIndex: 4, rotateX: isMobile ? 0 : 8 }
    } else if (diff === -2) {
      return { y: isMobile ? -180 : -280, scale: 0.7, opacity: 0.3, zIndex: 3, rotateX: isMobile ? 0 : 15 }
    } else if (diff === 1) {
      return { y: isMobile ? 100 : 160, scale: 0.82, opacity: 0.6, zIndex: 4, rotateX: isMobile ? 0 : -8 }
    } else if (diff === 2) {
      return { y: isMobile ? 180 : 280, scale: 0.7, opacity: 0.3, zIndex: 3, rotateX: isMobile ? 0 : -15 }
    } else {
      return { y: diff > 0 ? 400 : -400, scale: 0.6, opacity: 0, zIndex: 0, rotateX: isMobile ? 0 : (diff > 0 ? -20 : 20) }
    }
  }
  ...
  return (
    <div id="image-stack-container" className="relative flex h-[80vh] w-full items-center justify-center overflow-hidden bg-transparent">
      {/* Sapphire ambient glow - Removed on mobile */}
      {!isMobile && (
      ...
      {/* Card Stack */}
      <div className={`relative flex items-center justify-center ${isMobile ? 'h-[400px] w-[260px]' : 'h-[500px] w-[320px]'}`} style={{ perspective: "1200px" }}>
        {images.map((image, index) => {
          ...
          return (
            ...
            >
              <div
                className={`relative overflow-hidden rounded-[2rem] bg-zinc-950 border transition-colors duration-700 shadow-2xl ${isMobile ? 'h-[300px] w-[200px]' : 'h-[420px] w-[280px]'
      ...
      {/* Navigation dots - Hidden on small mobile */}
      {!isMobile && (
        <div className="absolute right-12 top-1/2 flex -translate-y-1/2 flex-col gap-3">
          {images.map((_, index) => (
            <button
              ...
              className={`w-1 rounded-full transition-all duration-700 ${index === currentIndex ? "h-12 bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.5)]" : "h-4 bg-white/10 hover:bg-white/30"
                }`}
