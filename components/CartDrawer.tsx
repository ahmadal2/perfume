          ...
          >
            ...
            <div className="px-8 py-10 flex items-center justify-between border-b border-white/5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-black uppercase tracking-[0.2em] serif italic text-white">{txt.title}</h2>
            ...
            {items.length > 0 && (
              <div className="p-8 border-t border-white/5 space-y-6 bg-[#0a0d14]/40 backdrop-blur-md">
                ...
                >
                  <span className="relative z-10">{txt.checkout_btn}</span>
...
*Order Summary:*
${items.map(item => `- ${item.name} (${item.size}) x${item.quantity}: $${(item.price * item.quantity).toFixed(2)}`).join('\n')}
...
Please guide me through the payment process.`;
                  ...
                  className="relative group block w-full text-center bg-[#25D366] mt-4 hover:bg-[#20bd5a] text-white py-5 rounded-2xl uppercase font-black tracking-[0.3em] text-[10px] transition-all duration-500 shadow-[0_15px_40px_rgba(37,211,102,0.2)] overflow-hidden"
                >
                  <span className="relative z-10 flex items-center justify-center gap-2">
                    <MessageCircle size={16} /> {txt.checkout_wa}
                  </span>
                </button>
